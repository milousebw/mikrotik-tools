...
# === Route SpeedTest vers IP publique fournie ===
@app.route('/speedtest', methods=['GET'])
def speedtest():
    target_ip = request.args.get('ip')       # IP publique du routeur à tester
    target_user = request.args.get('user')   # login vers routeur cible
    target_pass = request.args.get('pass')   # mot de passe routeur cible

    if not target_ip or not target_user or not target_pass:
        return "Paramètres requis : ip, user, pass", 400

    # Config de ta VM CHR (celle qui exécute le test)
    local_chrouter_ip = "192.168.0.252"
    local_user = "admin"
    local_pass = "FG95876"

    # Commande MikroTik à exécuter depuis la VM CHR
